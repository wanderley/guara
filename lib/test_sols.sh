...
if [ -e $special_judge ]; then
  special_judge_cmd=$(sh $script_dir/compile.sh $special_judge)
fi
...
for sol in $(ls sols/ | egrep "\-[0-9]+pts..{2,3}$" | egrep "(.c|.cpp|.rb|.py)"); do
  printf "%72s\n" $sol
  echo   "------------------------------------------------------------------------"

  sol_cmd=$(sh $script_dir/compile.sh sols/$sol)
